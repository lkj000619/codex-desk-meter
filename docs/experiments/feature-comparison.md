...
| F2 | 정규화·출처 분리 | 개인 사용량과 `codex-resets.com`을 공통 모델로 변환하고 출처를 보존하는가? (운반·파서 계층의 `codex-reset.com` 분리 보관은 호환 목적이며 이중 표시 요구가 아니다) | production parser host test, fixture 오류 시험, source 필드 | fixture parser 범위에서 평가 |
...
| G3 | 상태·출처 구분 | 상태/출처 혼동 | 라벨 일부만 구분 | 세 화면과 출처가 구분됨 | 확정·경과·개인·글로벌 의미가 즉시 구분됨 |
...
| G6 | 보드별 최적화 | 화면비·패널 특성 무시 | 잘림/성능 위험 존재 | 820×320 가로에 맞게 동작 | ST7701 초기화·가로 기본·메모리·렌더링을 근거와 함께 최적화; 선택적 회전 구현 시 정보 보존 |
...
- 패널은 가로, 관찰 거리 50cm, 정면에서 평가한다. 조명·화면 밝기·카메라 위치와
  노출을 첫 평가에 기록한 뒤 같은 cohort에서 유지한다. 조건이 바뀌면 별도 평가로 표시한다.
- 같은 fixture·기준 시각의 대시보드, 글로벌 리셋, 오류/stale/unknown 화면을 촬영한다.
  확대 촬영은 증거 보조용이며 50cm 가독성 판정을 대체하지 않는다.
- G1~G4는 provider/창, 잔여율, reset·조회 시각, 출처, 오류·미확인을 찾고 읽는
  동일 과제로 평가한다. 3점은 모든 대상이 5초 내 정확히 식별됨, 2점은 10초 내
  식별되나 일부 탐색 필요, 1점은 10초를 넘기거나 일부 오독, 0점은 핵심 정보
  식별 불가다. 표의 항목별 의미와 이 조건을 함께 적용해 더 낮은 점수를 기록한다.
- G5는 BOOT 전환과 PC 수동 갱신·실패를 같은 순서로 촬영하고 C8 시간 기준을
  함께 기록한다. G6의 선택적 IMU 회전 미구현만으로 감점하지 않으며, 구현했다면
  회전 후 정보 손실을 확인한다.
- 판정자는 모델 이름을 가린 run ID로 먼저 채점하고 항목별 사유를 기록한다.
  가능하면 두 판정자가 독립 채점하고, 2점 이상 차이는 근거를 비교해 조정한다.
